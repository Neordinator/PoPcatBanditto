...
Oatmeal::Oatmeal(const sf::Texture* _texture_base, const sf::Texture* _texture_prize
	, const sf::Vector2f& _position, const sf::Vector2f& _render_texture_size)
	: Element(_texture_base, _texture_prize, _position, _render_texture_size, EType::Oatmeal)
{
	setProperty();
	applyOffset();
}

void Oatmeal::setProperty()
{
